  ...
  std::thread([shared, powered, tech_path]() {
    try {
      ...
      proxy->callMethodAsync("SetProperty")
          ...
          .getResultAsFuture<>()
          .get();
      // NOTE: Calling Success from this background thread is unsafe
      // and will be fixed in Part 5.
...
4. **Call `Success` or `Error` on `shared`**. As discussed in Part 5, calling this directly from the background thread is unsafe and must be marshalled back to the platform thread.
