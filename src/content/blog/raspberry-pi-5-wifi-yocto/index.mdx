...
ConnMan exposes its full API over D-Bus, which is how programmatic clients (like embedded UIs) communicate with it. You can also exercise this API directly from the shell using `gdbus` or `dbus-send`.
...
gdbus call --system \
  --dest net.connman \
  --object-path / \
  --method net.connman.Manager.GetProperties
```
...
gdbus call --system \
  ...
  --method net.connman.Technology.SetProperty \
  "Powered" "<true>"
```

Or equivalently with `dbus-send`:
...
gdbus call --system \
  --dest net.connman \
  --object-path /net/connman/technology/wifi \
  --method net.connman.Technology.Scan
```
...
gdbus call --system \
  --dest net.connman \
  --object-path / \
  --method net.connman.Manager.GetServices
```
...
gdbus call --system \
  --dest net.connman \
  --object-path /net/connman/service/wifi_aabbccddee_<id>_managed_none \
  --method net.connman.Service.Connect
```
...
gdbus call --system \
  --dest net.connman \
  --object-path /net/connman/service/wifi_aabbccddee_4d794e6574776f726b_managed_psk \
  --method net.connman.Service.GetProperties
```
...
gdbus call --system \
  --dest net.connman \
  --object-path /net/connman/service/wifi_aabbccddee_<id>_managed_psk \
  --method net.connman.Service.Disconnect
```
